    ...
    <section class="crud">
      <div class="container">
        <form action="form.php" method="post">
          ...
          <button type="submit" id='forSub' class="btn btn-primary">Add</button>
        </form>
      </div>
    </section>
        ...
        <?php
          ...
          if($r > 0){
              while($value = mysqli_fetch_assoc($row)){
                echo '<tr scope="row" >';
                //echo '<td>' . $value . '</td>';
                echo '<td  scope="col">' . $value['id'] .'</td>' . '<td>'. $value['name'] .'</td>'.'<td>'. $value['email'] .'</td>'. '<td>'. $value['message'] .'</td>';
                echo '<td>'.'<button class="btn btn-success">Edit</button> <button class="btn mx-2 btn-primary">Delete</button> '.'</td>';
                echo '</tr>';
